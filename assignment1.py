...
def run_with_threads(tur, log, main_turtle):
    ...
    Shape1 = threading.Thread(target=draw_squares(tur), args= lock)
    Shape2 = threading.Thread(target=draw_circles(tur), args = lock)
    Shape3 = threading.Thread(target=draw_triangles(tur), args = lock)
    Shape4 = threading.Thread(target=draw_rectangles(tur), args = lock)
